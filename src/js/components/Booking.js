  ...
  initTables(event){
    ...
    if(clickedTable.classList.contains(classNames.booking.tableBooked)){
      ...
      if(!clickedTable.classList.contains(classNames.booking.tableSelected)){
        thisBooking.tableNumber = clickedTable.getAttribute(settings.bookings.tableIdAttribute);
      }
      thisBooking.removeSelected(clickedTable);
      clickedTable.classList.toggle(classNames.booking.tablePicked);
    }
    thisBooking.tableId = clickedTable.getAttribute(settings.bookings.tableIdAttribute);
  }
  removeSelected(clickedElement){
    const thisBooking = this;
    const clickedElementId = clickedElement.getAttribute(settings.bookings.tableIdAttribute);
    for(let table of thisBooking.dom.tables){
      const tableId = table.getAttribute(settings.bookings.tableIdAttribute);
